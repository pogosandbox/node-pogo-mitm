...
class Decoder {
    ...
    decodeRequest(session, requestId) {
        if (fs.existsSync(`data/${session}/${requestId}.req.json`, 'utf8')) {
            // decoded already, use it
            return fs.readFileAsync(`data/${session}/${requestId}.req.json`, 'utf8')
                    .then(data => this.fixLongToString(JSON.parse(data)));
        }
        ...
        .then(content => {
            ...
            return fs.writeFileAsync(`data/${session}/${requestId}.req.json`, JSON.stringify(data, null, 4), 'utf8')
                    .then(() => this.fixLongToString(data));
        });
    }

    decodeResponse(session, requestId) {
        if (fs.existsSync(`data/${session}/${requestId}.res.json`, 'utf8')) {
            // decoded already, use it
