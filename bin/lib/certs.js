...
class MyCerts {
    myCertGenerator(ca, hosts, cb) {
        ...
        fs.writeFile(ca.certsFolder + '/' + mainHost.replace(/\*/g, '_') + '.pem', certPem, function (error) {
            if (error)
                console.error('Failed to save certificate to disk in ' + ca.certsFolder, error);
        });
        fs.writeFile(ca.keysFolder + '/' + mainHost.replace(/\*/g, '_') + '.key', keyPrivatePem, function (error) {
            if (error)
                console.error('Failed to save private key to disk in ' + ca.keysFolder, error);
        });
        fs.writeFile(ca.keysFolder + '/' + mainHost.replace(/\*/g, '_') + '.public.key', keyPublicPem, function (error) {
            if (error)
                console.error('Failed to save public key to disk in ' + ca.keysFolder, error);
        });
