...
export default class FakeServer {
    ...
    bufEquals(buf, index, bytes) {
        if (index + bytes.length > buf.length) return false;
    ...
    readHeader(buffer) {
        ...
        if (buffer.readUInt16LE(index) !== 8) throw new Error('Unexpected value at ' + index);
        index += 2;

        if (buffer.readUInt16LE(index) !== 4) throw new Error('Unexpected value at ' + index);
        index += 2;

        let byte = buffer.readUInt16LE(index);
        if (type === 'request' && byte !== 0) throw new Error('Unexpected value at ' + index);
        else if (type === 'response' && byte !== 0x0F) throw new Error('Unexpected value at ' + index);
        index += 2;

        byte = buffer.readUInt32LE(index);
        if (byte !== 0x0E) throw new Error('Unexpected value at ' + index);
        index += 4;

        typeId = buffer.readUInt16LE(0x1c);
        if (typeId !== 1 && typeId !== 2) throw new Error('Unexpected typeId value');
    ...
    async handleRequest(requestId: string, buffer: Buffer): Promise<Buffer> {
        // for debugging
        await fs.writeFile(`${this.config.datadir}/${requestId}.req.raw`, buffer);
        ...
        if (header.type !== 'request') throw new Error('Unexpected request type. request was expected, got ' + header.type);
